...
if 'df2009' in st.session_state:
    ...
    if st.button('Click here to for the 5 - year forecast.'):
        ...
        # Check if county_df is empty or has insufficient data
        if county_df.empty or county_df[topic_title].isna().all():
            st.error(f"No data available for {chosen_topic} in {chosen_county}. Please select a different county or health indicator.")
        elif county_df[topic_title].count() < 4:
            st.error(f"Insufficient data points for {chosen_topic} in {chosen_county}. At least 4 non-null data points are required for forecasting.")
        else:
